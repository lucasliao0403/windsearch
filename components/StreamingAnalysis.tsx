...
interface StreamingAnalysisProps {
  ...
  isFollowUp?: boolean; // Follow-up queries get different treatment
}

export default function StreamingAnalysis({ query, stations, onChartsReceived, isActive = true, isFollowUp = false }: StreamingAnalysisProps) {
              ...
              try {
                ...
                if (jsonData) {
                  ...
                  if (data.type === 'charts') {
                    ...
                    // Only process chart data for initial queries, not follow-ups
                    if (!isFollowUp) {
    ...
    <div className="space-y-6 animate-in slide-in-from-bottom-8 duration-700">
      ...
      <div className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 border border-gray-700 hover:bg-gray-800/60 hover:border-gray-600 transition-all duration-500 ease-out">
        <div className="flex items-center gap-3 mb-4">
          <h3 className="text-xl font-semibold">{isFollowUp ? 'Follow-up' : 'Quick Analysis'}</h3>
          {isLoading && (
      ...
      {/* Detailed Summary - only show for initial queries, not follow-ups */}
      {!isFollowUp && (summary || isGeneratingSummary) && (
